...
const (
	...
	validatorPrefix = 0x12

	userTypesPrefix = 0x13
...
func ModifyBalance(db database.KeyValueReaderWriter, address common.Address, add bool, change uint64) (uint64, error) {
	...
	if xflow {
		return 0, fmt.Errorf("%w: modify bal=%d, addr=%v, add=%t, prev=%d, change=%d", ErrInvalidBalance, b, address, add, b, change)
	}
	return n, SetBalance(db, address, n)
}

func GetStakeBalance(db database.KeyValueReader, address common.Address) (uint64, error) {
	k := PrefixStakeKey(address)
...
func ModifyStakeBalance(db database.KeyValueReaderWriter, address common.Address, add bool, change uint64) (uint64, error) {
	b, err := GetStakeBalance(db, address)
	if err != nil {
		return 0, err
	}
	var (
	...
	if xflow {
		return 0, fmt.Errorf("%w: lockerr bal=%d, addr=%v, add=%t, prev=%d, change=%d", ErrInvalidBalance, b, address, add, b, change)
	}
	return n, SetStakeBalance(db, address, n)
}
