...
type PushNetwork struct {
	vm          *VM
	gossipedTxs *cache.LRU[ids.ID, struct{}]
}

func (vm *VM) NewPushNetwork() *PushNetwork {
	return &PushNetwork{
		vm:          vm,
		gossipedTxs: &cache.LRU[ids.ID, struct{}]{Size: gossipedTxsLRUSize},
	}
}
...
func (n *PushNetwork) GossipNewTxs(newTxs []*chain.Transaction) error {
	...
	for _, tx := range newTxs {
		...
		n.gossipedTxs.Put(tx.ID(), struct{}{})
		txs = append(txs, tx)
	}
...
// the other VM (samavm)  via "common.AppSender" to receive txs and
...
func (vm *VM) AppGossip(ctx context.Context, nodeID ids.NodeID, msg []byte) error {
	...
	if errs := vm.Submit(nil, txs...); len(errs) > 0 {
