...
import (
	...
	"github.com/SamaNetwork/SamaVM/mempool"
...
func (vm *VM) Initialize(
	...
	log.Info("initializing samavm", "version", version.Version)
	...
	//ctx.Keystore = ks.NewBlockchainKeyStore(vm.ctx.ChainID)
	...
	vm.genesis = chain.DefaultGenesis()
	...
	if has { //nolint:nestif
		...
		log.Info("initialized samavm from last accepted", "block", blkID)
		...
		log.Info("initialized samavm from genesis", "block", gBlkID)
	}
	registerer := prometheus.NewRegistry()

	state, err := chain.SamaNew(vm.db, registerer, vm.genesis)
	if err != nil {
		return err
	}
	vm.samaState = state
...
// implements "snowmanblock.ChainVM.common.VM"
func (vm *VM) Version(context.Context) (string, error) { return version.Version.String(), nil }
...
func (vm *VM) CreateHandlers(_ context.Context) (map[string]*common.HTTPHandler, error) {
	...
	public, err := newHandler(Name, &PublicService{vm: vm, Cache: make(map[string]*LocalParams)})
	if err != nil {
		return nil, err
	}
...
func (vm *VM) Submit(_ context.Context, txs ...*chain.Transaction) (errs []error) {
...
func (vm *VM) submit(tx *chain.Transaction, db database.Database, blkTime int64, ctx *chain.Context) error {
	...
	dummy := chain.DummyBlock(blkTime, tx, vm)
