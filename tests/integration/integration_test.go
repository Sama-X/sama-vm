...
func TestIntegration(t *testing.T) {
	...
	ginkgo.RunSpecs(t, "samavm integration test suites")
}
...
var _ = ginkgo.Describe("Tx Types", func() {
	...
	ginkgo.It("Gossip SetTx to a different node", func() {
		...
		ginkgo.By("build block in the node 1", func() {
			blk, err := instances[1].vm.BuildBlock(nil)
			gomega.Ω(err).To(gomega.BeNil())

			gomega.Ω(blk.Verify(nil)).To(gomega.BeNil())
			gomega.Ω(blk.Status()).To(gomega.Equal(choices.Processing))

			err = instances[1].vm.SetPreference(nil, blk.ID())
			gomega.Ω(err).To(gomega.BeNil())

			gomega.Ω(blk.Accept(nil)).To(gomega.BeNil())
			gomega.Ω(blk.Status()).To(gomega.Equal(choices.Accepted))

			lastAccepted, err := instances[1].vm.LastAccepted(nil)
			gomega.Ω(err).To(gomega.BeNil())
...
func asyncBlockPush(i instance, c chan struct{}) {
	...
	for {
		...
		case <-timer.C:
			...
			err = i.vm.SetPreference(nil, blk.ID())
			gomega.Ω(err).To(gomega.BeNil())

			gomega.Ω(blk.Accept(nil)).To(gomega.BeNil())
			gomega.Ω(blk.Status()).To(gomega.Equal(choices.Accepted))

			lastAccepted, err := i.vm.LastAccepted(nil)
			gomega.Ω(err).To(gomega.BeNil())
			gomega.Ω(lastAccepted).To(gomega.Equal(blk.ID()))
		}
	}
